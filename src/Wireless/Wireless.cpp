...
int wifi_scan_number()
{
  ...
  // CRITICAL: Disable WiFi sleep mode for maximum scanning reliability
  WiFi.setSleep(false);

  // Set WiFi to maximum transmit power for better signal reception
  WiFi.setTxPower(WIFI_POWER_19_5dBm);  // Maximum power

  // Disable power saving mode for best performance during scan
  ...
  int count = WiFi.scanNetworks(false, true, false, 1000);
  if (count == 0)
  {
